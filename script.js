...
const State = (() => {
    ...
    return {
        ...
        setCurrentMonth(date)  { _currentMonth = new Date(date); },
...
const Utils = {
    ...
    add15Mins(time12) {
        ...
        return Utils.to12Hour(`${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`);
    },
...
const Validator = {
    validate(data, editId = null) {
        ...
        if (!patientName) {
            ...
            errors['err-patient'] = 'Patient name must contain letters only — no numbers or special characters.';
        ...
        const reasonVal = (data.reason || '').trim();
        if (reasonVal && /[^A-Za-z0-9\s]/.test(reasonVal)) {
            errors['err-reason'] = 'Reason can only contain letters and numbers — no special characters.';
        }
    ...
    showErrors(errors) {
        ...
        document.querySelectorAll('.input-group input, .input-group select').forEach(el => el.classList.remove('error'));
        ...
        Object.entries(errors).forEach(([id, msg]) => {
            ...
            if (input) {
                ...
                if (group) {
                    ...
                    // textarea (e.g. reason) not inside .input-group
                    input.classList.add('error');
                }
            }
        });
    ...
    clearErrors() {
        document.querySelectorAll('.field-error').forEach(el => el.textContent = '');
        document.querySelectorAll('.input-group .error, textarea.error').forEach(el => el.classList.remove('error'));
    },
};
...
const Confirm = (() => {
    ...
    function close() { overlay.hidden = true; if (_resolve) _resolve(false); }
...
const Layout = (() => {
    ...
    const toggleMobile  = document.getElementById('btn-toggle-mobile');
    const toggleDesktop = document.getElementById('btn-toggle-desktop');
    ...
    function closeMobileDrawer() {
        ...
        if(toggleMobile) toggleMobile.setAttribute('aria-expanded', 'false');
    }

    function toggleSidebarMobile() {
        const open = sidebar.classList.toggle('mobile-open');
        overlay.style.display = open ? 'block' : 'none';
        toggleMobile.setAttribute('aria-expanded', String(open));
    }

    // FIX: Removed the line that collapsed the header right-side
    ...
    function init() {
        if (toggleMobile) toggleMobile.addEventListener('click', toggleSidebarMobile);
        if (toggleDesktop) toggleDesktop.addEventListener('click', toggleSidebarDesktop);
...
const Calendar = (() => {
    ...
    const datePicker      = document.getElementById('cal-native-date-picker');
        ...
        shown.forEach(appt => {
            ...
            ev.innerHTML = `
                ...
                <div class="event-actions" aria-hidden="true">
                    <svg class="evt-edit" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" title="Edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    <svg class="evt-delete" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" title="Delete"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                </div>`;

            ev.addEventListener('click',  e => { e.stopPropagation(); Modal.open(appt.id); });
            ev.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); Modal.open(appt.id); } });
    ...
    function render() {
        ...
        const day   = cm.getDate();

        // Exact display mapping to "January 18, 2023"
        if (fullDateDisplay) fullDateDisplay.textContent = `${CONFIG.MONTH_NAMES[month]} ${day}, ${year}`;
        if (monthSelect) monthSelect.value = month;
        if (datePicker) datePicker.value = Utils.toDateStr(year, month, day);
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const dayOfWeek = CONFIG.DAY_NAMES[new Date(year, month, d).getDay()];
    ...
    function init() {
        ...
        datePicker?.addEventListener('change', e => {
            ...
            State.setCurrentMonth(new Date(parseInt(y), parseInt(m) - 1, parseInt(d)));
            render();
        });

        const dateDisplayBtn = document.getElementById('date-display-btn');
        dateDisplayBtn?.addEventListener('click', () => {
            try { if (datePicker) datePicker.showPicker(); } 
            catch (e) { if (datePicker) datePicker.focus(); }
        });
    }
...
const Dashboard = (() => {
    ...
    function buildTableRow(appt) {
        ...
        tr.innerHTML = `
            ...
            <td class="actions-cell">
                <button class="tbl-action edit"   data-id="${appt.id}" title="Edit appointment">${Utils.svgEdit}</button>
                <button class="tbl-action delete" data-id="${appt.id}" title="Delete appointment">${Utils.svgDelete}</button>
            </td>`;
        return tr;
    }
        ...
        card.innerHTML = `
            <div class="appt-card-header">
                ...
                <div class="event-actions">
                    <button class="tbl-action evt-edit edit" data-id="${appt.id}">${Utils.svgEdit}</button>
                    <button class="tbl-action evt-delete delete" data-id="${appt.id}">${Utils.svgDelete}</button>
                </div>
            </div>
            <div class="appt-card-body">
                <div class="appt-card-field"><div class="appt-card-label">Doctor</div><div class="appt-card-value text-blue">${Utils.esc(appt.doctor)}</div></div>
                <div class="appt-card-field"><div class="appt-card-label">Specialty</div><div class="appt-card-value">${Utils.esc(appt.specialty)}</div></div>
                <div class="appt-card-field"><div class="appt-card-label">Date</div><div class="appt-card-value">${displayDate}</div></div>
                <div class="appt-card-field"><div class="appt-card-label">Time</div><div class="appt-card-value text-blue">${Utils.esc(displayTime)} - ${Utils.esc(timeEnd)}</div></div>
                <div class="appt-card-field" style="grid-column:1/-1"><div class="appt-card-label">Hospital</div><div class="appt-card-value">${Utils.esc(appt.hospital)}</div></div>
            </div>`;
        return card;
    }
    ...
    async function handleAction(e) {
        ...
        if (editBtn || linkEdit) {
            e.preventDefault();
            Modal.open((editBtn || linkEdit).getAttribute('data-id'));
            return;
        }
        ...
        if (deleteBtn) {
            ...
            if (await Confirm.ask(`Delete ${appt.patient}'s appointment on ${Utils.formatDate(appt.date)}?`)) {
    ...
    function render() {
        ...
        resultCount.textContent = filtered.length === total ? `${total} appointment${total !== 1 ? 's' : ''}` : `${filtered.length} of ${total}`;
    ...
    function init() {
        ...
        document.getElementById('btn-update-filters')?.addEventListener('click', () => { syncFiltersFromUI(); render(); });
...
const Modal = (() => {
    ...
    async function handleSave() {
        ...
        const data = { ...raw, time: Utils.to12Hour(raw.time) };
        const { valid, errors } = Validator.validate(raw, _editId);
        
        if (!valid) {
            Validator.showErrors(errors);
            document.getElementById(Object.keys(errors)[0].replace('err-', 'appt-'))?.focus();
            return;
        }
    ...
    function init() {
        ...
        form.addEventListener('input', e => {
            const err = document.getElementById(`err-${e.target.id.replace('appt-', '')}`);
